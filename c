 ...
 *      and assigns it a conflict-free faculty proctor.
 *
 *   2. Detects existing faculty-proctor conflicts (same proctor double-booked
 *      at overlapping times) and resolves them by reassigning one of the
 *      conflicting exams to a free faculty member.
 *
 ...
 *   - a proctor may NEVER be assigned to two exams at overlapping times
 *     on the same date (a hard "no conflict" guarantee)
...
// date/time/room/day are ALL "TBA".  These are NOT real exams and should
// never get a proctor nor participate in conflict checks.
...
function isRealDate(v) {
  ...
  return true; // any non-TBA token counts as a real date
}
...
async function loadFaculty() {
  ...
  facultySnap.docs.map((d) => ({ id: d.id, ...d.data() })).forEach(push);
    ...
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        ...
        if (a.section && a.section === b.section) {
          // Allow same section only if same subject+code in same schedule
          // (duplicate entry, not a real conflict).
...
// ---------------------------------------------------------------------------
// Proctor-schedule tracker (shared by assignment & resolution)
// ---------------------------------------------------------------------------
// Returns a Map: normalizedName -> { date -> [ {time, section, code} ] }
function buildProctorSchedule(bookings) {
...
function isFree(proctorDaily, facultyName, date, time) {
...
function dayCount(proctorDaily, facultyName, date) {
  const key = normaliseName(facultyName);
  const perDate = proctorDaily.get(key);
  return perDate ? (perDate.get(date) || []).length : 0;
}

function totalCount(proctorDaily, facultyName) {
  const key = normaliseName(facultyName);
  const perDate = proctorDaily.get(key);
  if (!perDate) return 0;
  let n = 0;
...
function addAssignment(proctorDaily, facultyName, date, time, section, code) {
  const key = normaliseName(facultyName);
  const perDate = proctorDaily.get(key) || new Map();
  const arr = perDate.get(date) || [];
...
function removeAssignment(proctorDaily, facultyName, date, time, section, code) {
  const key = normaliseName(facultyName);
  const perDate = proctorDaily.get(key);
  if (!perDate) return;
  const arr = perDate.get(date);
...
// Pick the best free faculty for a given (date, time), excluding any names in
// `excludeNames`. Respects the 2-per-day cap; falls back to overlap-only if no
// candidate is under the cap.
function pickFreeFaculty(proctorDaily, faculty, date, time, excludeNames = []) {
  const exclude = new Set(excludeNames.map(normaliseName));
  const candidates = faculty.filter(
    (f) => !exclude.has(normaliseName(f.fullName))
  );

  // Prefer candidates under the per-day cap who are overlap-free.
  let pool = candidates.filter(
    (f) => isFree(proctorDaily, f.fullName, date, time) &&
      dayCount(proctorDaily, f.fullName, date) < MAX_EXAMS_PER_PROCTOR_PER_DAY
  );
  let relaxed = false;
  if (!pool.length) {
    // Relax the cap (still never allow a true overlap).
    relaxed = true;
    pool = candidates.filter((f) => isFree(proctorDaily, f.fullName, date, time));
  }

  if (!pool.length) return null;

  pool.sort((a, b) => {
    const da = dayCount(proctorDaily, a.fullName, date) - dayCount(proctorDaily, b.fullName, date);
    if (da !== 0) return da;
    return totalCount(proctorDaily, a.fullName) - totalCount(proctorDaily, b.fullName);
  });
  return { ...pool[0], relaxed };
}

// ---------------------------------------------------------------------------
// Phase 2a: assign TBA proctors
// ---------------------------------------------------------------------------
function assignTbaProctors(bookings, faculty) {
  const proctorDaily = buildProctorSchedule(bookings);
  const results = [];
  let assigned = 0;
  let unassigned = 0;
  ...
  for (const b of pending) {
    ...
    const picked = pickFreeFaculty(proctorDaily, faculty, b.date, b.time);
    if (picked) {
      b.proctor = picked.fullName;
      b.proctorUid = picked.uid;
      addAssignment(proctorDaily, picked.fullName, b.date, b.time, b.section, b.code);
      assigned++;
      results.push({ ...b, assignedProctor: picked.fullName, proctorUid: picked.uid, reason: null });
    } else {
      unassigned++;
      unassignedList.push(b);
      results.push({ ...b, assignedProctor: null, reason: "No free faculty available for this slot" });
    }
  }
...
// ---------------------------------------------------------------------------
// Phase 2b: resolve existing proctor conflicts
// ---------------------------------------------------------------------------
function resolveProctorConflicts(bookings, faculty) {
  const proctorDaily = buildProctorSchedule(bookings);
  const results = [];

  // Keep resolving until no proctor conflicts remain (each iteration may fix
  // a conflict but we rebuild the proctor schedule each pass to stay accurate).
  let pass = 0;
  while (pass < 50) {
    ...
    for (const c of conflicts) {
      ...
      // Remove b's current (conflicting) proctor from the schedule temporarily
      removeAssignment(proctorDaily, b.proctor, b.date, b.time, b.section, b.code);

      const picked = pickFreeFaculty(
        proctorDaily,
        faculty,
        b.date,
        b.time,
        [a.proctor] // don't put b back on the same colliding proctor
      );
      ...
      if (picked) {
        ...
        // Could not resolve — put the original assignment back so we don't lose it
        addAssignment(proctorDaily, b.proctor, b.date, b.time, b.section, b.code);
        results.push({
          scheduleId: b.scheduleId,
          idx: b.idx,
          section: b.section,
  ...
  return { results, resolved: results.filter((r) => r.resolved).length, unresolved: results.filter((r) => !r.resolved).length };
}
...
async function main() {
  ...
  log(`  Real exams requiring a slot (room/date/time): ${bookings.length}`);

  // --- Identify unassigned proctors among real exams ---
  const unassignedExams = bookings.filter((b) => isUnassignedProctor(b.proctor));
  const alreadyAssigned = bookings.filter((b) => !isUnassignedProctor(b.proctor));
  ...
  // --- Phase 1: pre-assignment conflict scan ---
  log("\n" + "-".repeat(72));
  log("PHASE 1: Pre-assignment conflict scan (before any fixes)");
  log("-".repeat(72));
  const preConflicts = detectConflicts(bookings);
  const preProctor = preConflicts.filter((c) => c.type === "PROCTOR_CONFLICT");
  const preRoom = preConflicts.filter((c) => c.type === "ROOM_CONFLICT");
  const preSection = preConflicts.filter((c) => c.type === "SECTION_CONFLICT");
  if (preConflicts.length === 0) {
    ...
    log(`      - Room conflicts:     ${preRoom.length}`);
    log(`      - Proctor conflicts:  ${preProctor.length}`);
    log(`      - Section conflicts:  ${preSection.length}`);
  ...
  // --- Phase 2a: assign TBA proctors ---
  log("\n" + "-".repeat(72));
  log("PHASE 2a: Assigning faculty proctors to TBA exams");
  log("-".repeat(72));
  const assignment = assignTbaProctors(bookings, faculty);
  ...
  // --- Phase 3: post-assignment conflict scan ---
  log("\n" + "-".repeat(72));
  log("PHASE 3: Final conflict scan (after all fixes)");
  log("-".repeat(72));
  const postConflicts = detectConflicts(bookings);
  const postProctor = postConflicts.filter((c) => c.type === "PROCTOR_CONFLICT");
  const postRoom = postConflicts.filter((c) => c.type === "ROOM_CONFLICT");
  const postSection = postConflicts.filter((c) => c.type === "SECTION_CONFLICT");
  if (postConflicts.length === 0) {
    log("  ✓ No conflicts remain (room, time, and faculty proctor).");
  } else {
    log(`  ⚠ Found ${postConflicts.length} conflict(s) after fixes:`);
    log(`      - Room conflicts:     ${postRoom.length}`);
    log(`      - Proctor conflicts:  ${postProctor.length}`);
    log(`      - Section conflicts:  ${postSection.length}`);
  ...
  log(`  Real exams:                 ${bookings.length}`);
  ...
  log(`  Conflicts after fix:        ${postConflicts.length} (room: ${postRoom.length}, proctor: ${postProctor.length}, section: ${postSection.length})`);

  // --- Phase 4: write back to Firestore ---
  // Collect ALL modifications (TBA assignments + conflict resolutions) grouped
  // by schedule document, preserving the original exams-array structure.
  const modificationsByDoc = new Map(); // docId -> { sched, examsCopy }

  const collect = (docId, idx, proctor, proctorUid) => {
    if (!modificationsByDoc.has(docId)) {
      const sched = schedules.find((s) => s.id === docId);
      if (!sched) return;
      const examsCopy = Array.isArray(sched.exames) ? sched.exames : [];
      modificationsByDoc.set(docId, { sched, examsCopy });
    }
    const { examsCopy } = modificationsByDoc.get(docId);
    if (examsCopy[idx]) {
      examsCopy[idx] = { ...examsCopy[idx], proctor, proctorUid };
    }
  };

  for (const r of assignment.results) {
    if (r.assignedProctor) {
      collect(r.scheduleId, r.idx, r.assignedProctor, r.proctorUid || "");
    }
  }
  for (const r of resolution.results) {
    if (r.resolved) {
      collect(r.scheduleId, r.idx, r.newProctor, r.proctorUid || "");
    }
  }

  if (modificationsByDoc.size > 0) {
    log("\n" + "-".repeat(72));
    log("PHASE 4: Persisting fixed proctor assignments to Firestore");
    log("-".repeat(72));

    let writtenDocs = 0;
    for (const [docId, { sched, examsCopy }] of modificationsByDoc) {
      const toWrite = {
        ...sched,
        exams: examsCopy,
